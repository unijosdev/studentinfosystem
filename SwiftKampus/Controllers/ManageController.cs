...
namespace SwiftKampus.Controllers
{
    ...
    public class ManageController : BaseController
    {
        ...
        public ManageController(ApplicationUserManager userManager,
            ApplicationSignInManager signInManager, SchoolDbContext db):base(db)
        {
        ...
        public async Task<ActionResult> Index(ManageMessageId? message)
        {
            ViewBag.StatusMessage =
                ...
                : "";
            var myuserId = _db.Users.AsNoTracking().Where(x => x.Email.Equals(userId)).Select(s => s.Id).FirstOrDefault();
        ...
        public async Task<ActionResult> VerifyPhoneNumber(VerifyPhoneNumberViewModel model)
        {
            ...
            if (result.Succeeded)
            {
                ...
                if (User.IsInRole(RoleName.Academic) || User.IsInRole(RoleName.None_Academic))
                {
                    var id = User.Identity.GetUserId();
                    Staff myStaff = await _db.Staffs.FindAsync(id);
                    if (myStaff != null)
                    {
                        myStaff.PhoneNumber = model.PhoneNumber;
                        _db.Entry(myStaff).State = EntityState.Modified;
                    }
                ...
                {
                    var id = User.Identity.GetUserId();
                    Student myStudent = await _db.Students.FindAsync(id);
                    if (myStudent != null)
                    {
                        myStudent.PhoneNumber = model.PhoneNumber;
        ...
        public ActionResult EnableGoogleAuthenticator()
        {
            ...
            string appName = $"&issuer={@System.Configuration.ConfigurationManager.AppSettings["SchoolName"]}";
            string barcodeUrl = KeyUrl.GetTotpUrl(secretKey, userName) + appName;
