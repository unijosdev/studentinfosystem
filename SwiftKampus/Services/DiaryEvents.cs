...
namespace SwiftKampus.Services
{
    public class DiaryEvent
    {
        ...
        public static List<DiaryEvent> LoadAllAppointmentsInDateRange(double start, double end)
        {
            ...
            using (SchoolDbContext ent = new SchoolDbContext())
            {
                ...
                foreach (var item in rslt)
                {
                    DiaryEvent rec = new DiaryEvent
                    {
                        ...
                        StatusString = Enums.GetName<AppointmentStatus>((AppointmentStatus)item.StatusENUM)
                    };
                    rec.StatusColor = Enums.GetEnumDescription<AppointmentStatus>(rec.StatusString);
                    string ColorCode = rec.StatusColor.Substring(0, rec.StatusColor.IndexOf(":", StringComparison.CurrentCulture));
                    rec.ClassName = rec.StatusColor.Substring(rec.StatusColor.IndexOf(":", StringComparison.CurrentCulture) + 1, rec.StatusColor.Length - ColorCode.Length - 1);
        ...
        public static List<DiaryEvent> LoadAppointmentSummaryInDateRange(double start, double end)
        {
            ...
            using (SchoolDbContext ent = new SchoolDbContext())
            {
                ...
                foreach (var item in rslt)
                {
                    DiaryEvent rec = new DiaryEvent
                    {
                        ID = i, //we dont link this back to anything as its a group summary but the fullcalendar needs unique IDs for each event item (unless its a repeating event)
        ...
        public static void UpdateDiaryEvent(int id, string NewEventStart, string NewEventEnd)
        {
            ...
            using (var ent = new SchoolDbContext())
            {
