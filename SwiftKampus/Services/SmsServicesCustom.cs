...
namespace SwiftKampus.Services
{
    public class SmsServicesCustom
    {
        ...
        private readonly string sessionId_cahe_key = "SmsSessionId_" + "GetSessionId";
        ...
        public string GetSessionId()
        {
            ...
            else
            {
                ...
                result = GetResponseMessage(response, out success, out string errMsg);
        ...
        public string GetResponseMessage(string response, out bool success, out string errMsg)
        {
            ...
            bool isSuccess = response.Substring(0, response.IndexOf(":", StringComparison.CurrentCulture) + 1).Contains("OK");
            ...
            if (isSuccess)
            {
                code = response.Substring(response.IndexOf(":", StringComparison.CurrentCulture) + 2);
            }
            else
            {
                code = response.Substring(response.IndexOf(":", StringComparison.CurrentCulture) + 2,
                    response.LastIndexOf(":", StringComparison.CurrentCulture) - 1);
                errDesc = response.Substring(response.LastIndexOf(":", StringComparison.CurrentCulture) + 2);
            }
